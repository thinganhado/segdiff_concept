...
def produce_evaluation_file(dataset, model, device, save_path):
    ...
    for batch_x,utt_id in data_loader:
        ...
        # >>> dump exact model inputs for later comparison, plus optional PNGs
        import os, numpy as np
        from pathlib import Path

        dump_dir = Path(os.getenv("INPUT_DUMP_DIR", "/home/opc/asvspoof/wave_cache_modeldump"))
        dump_dir.mkdir(parents=True, exist_ok=True)

        save_png = os.getenv("DUMP_WAVE_PNG", "0") == "1"
        if save_png:
            # lazy import to avoid adding a hard dependency when PNGs are not requested
            import matplotlib
            matplotlib.use("Agg")
            import matplotlib.pyplot as plt

        # move to cpu for saving, squeeze channel if present
        bx_cpu = batch_x.detach().float().cpu()
        for i, uid in enumerate(utt_id):
            arr = bx_cpu[i].numpy()
            if arr.ndim == 2 and arr.shape[0] == 1:
                arr = arr[0]

            # save the exact tensor the model sees
            np.save(str(dump_dir / f"{uid}.npy"), arr)

            # optional waveform PNG
            if save_png:
                # simple waveform plot, axes off for compact files
                fig = plt.figure(figsize=(6, 2), dpi=120)
                ax = fig.add_subplot(111)
                ax.plot(arr, linewidth=0.6)
                ax.set_xticks([]); ax.set_yticks([])
                ax.set_xlim(0, len(arr))
                for spine in ax.spines.values():
                    spine.set_visible(False)
                fig.tight_layout(pad=0.05)
                fig.savefig(dump_dir / f"{uid}.png")
                plt.close(fig)
        # <<< end dump block
...
if __name__ == '__main__':
    ...
    # <<< add these lines, after loading weights, before eval starts
    debug_dump_inputs.attach(model, protocols_path=args.protocols_path)
    # >>>
